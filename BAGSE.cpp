...
//' An Rcpp Armadillo function to find log10 Bayes factors 
//'
...
// [[Rcpp::export]] 
colvec compute_log10BF(double bhat, double sdhat, colvec& phi_vec) {
	int p = phi_vec.size();
	colvec val(p);
	for (int i = 0; i < p;i++) {
...
// [[Rcpp::export]] 
colvec computegridwts(colvec& log10_BF_vec, colvec& pi_vec, double pi0, double log10_NC) {
	int i,j;
	double sum = 0;
...
//' An Rcpp Armadillo function to calculate enrichment parameters, local FDR for all genes, and estimates for the weights of the mixture normal for 
//' @param bethat        An n length column vector containing effect size estimates for all n genes
...
// [[Rcpp::export]] 
List torus_cpp(colvec& betahat, colvec& sebetahat, uvec& annotation, double tol = 1e-1) {
